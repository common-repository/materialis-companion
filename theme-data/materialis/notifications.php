...
function extendthemes_remote_notifications_handle( $data ) {
	$url = 'https://extendthemes.com/materialis/';
	if ( $data['coupon_code'] ) {
	...
	<div class="extendthemes-remote-notification extendthemes-remote-notification-<?php echo esc_attr( $data['notification_id'] ); ?>">
		...
		<style>
			[data-cp-notification-name="extendthemes_remote_notification_materialis_<?php echo esc_attr( $data['notification_id'] ); ?>"] {
				background-image: url("<?php echo esc_url( $data['background_image'] ); ?>");
				background-size: cover;
				background-repeat: no-repeat;
			}

			[data-cp-notification-name="extendthemes_remote_notification_materialis_<?php echo esc_attr( $data['notification_id'] ); ?>"] .message p:first-child {
				font-size: 20px;
			}

			<?php echo str_replace( '#ID', "[data-cp-notification-name=\"extendthemes_remote_notification_materialis_{$data['notification_id']}\"]", $data['custom_css'] ); ?>
		</style>
	</div>
	<?php
}
...
return array(
	array(
		...
		'props'           => array(
			'message' => "Winter Holidays Special Offer - <span style='color:red'>20% discount </span> for <strong>Materialis PRO</strong>",
			'buttons' => array(
				array(
					'class' => 'primary main',
					'link'  => 'https://extendthemes.com/mesmerize/#pricing',
					'label' => 'Get the offer',
				),
				array(
					'class' => 'primary',
					'label' => 'See PRO Features',
					'link'  => 'https://extendthemes.com/mesmerize/#content-1',
				),
			),
		),
	...
	array(
		'name'            => 'summer_sale_2018',
		'dismissible'     => true,
		'start'           => '10-08-2018',
		'end'             => '27-08-2018',
		'type'            => 'cp-announcement ',
		'handle'          => 'cp_notification_discount_offer',
		'active_callback' => 'cp_notification_winter_holiday_2017_active_callback',
		'props'           => array(
			'message' => "Summer Sale - <span style='color:red'>20% discount </span> for all <strong>Materialis PRO</strong> licenses",
			'buttons' => array(
				array(
					'class' => 'primary main',
					'link'  => 'https://extendthemes.com/mesmerize/#pricing',
